...
```bash
npm install -g https://www.myfundfarm.com/cli/fundfarm-cli-0.2.6.tgz
fundfarm login
fundfarm whoami
```
...
| `fundfarm trade buy <代码> --amount <金额> --time <交易时间> --yes` | 买入基金（`--time` 为 Agent 下单时间） |
| `fundfarm trade sell <代码> --shares <份额> --time <交易时间> --yes` | 卖出基金（`--time` 为 Agent 下单时间） |
...
- **交易时间语义**：`trade buy/sell --time` 表示 Agent 下单时间，仅用于自动判断净值归属日与 pending；主站用户手动录入交易时，仍以用户自己选择的交易日期和净值为准
...
其中：
- `--time` / `transaction_time` = Agent 下单时间
- `transaction_date` = 净值归属日
- `created_at` = 记录写入时间（主站网页手动录入时仅作审计/展示，不参与净值判断）
