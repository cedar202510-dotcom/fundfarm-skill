...
```bash
npm install -g https://www.myfundfarm.com/cli/fundfarm-cli-0.2.1.tgz
fundfarm login
fundfarm whoami
```
...
| `fundfarm holdings import <代码> --market-value <市值> --profit <收益> --yes` | 导入单个持仓 |
| `fundfarm holdings batch-import <file> --yes` | 批量导入持仓 |
...
所有写操作（trade、holdings import、watchlist add/remove、ai analyze）具有**多层安全保护**：
...
- **导入确认**：导入前会检查现有持仓，并明确提示是 “Agent 导入” 还是 “覆盖并记为 Agent 修改”
...
用户: 导入我现有的 161725 持仓 → fundfarm holdings import 161725 --market-value 12000 --profit 800 --yes 或 import_holding(...)
